...
class Client extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.session = new Session(opts.sessionOptions);
  }
  ...
  async addConnection() {
    if (!this.session.connected) throw new ErrorCode('Not connected', 'NOT_CONNECTED');
    let connection = new ClientConnection({
      host: this.host,
      port: this.port,
...
class ClientConnection extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.debugLog(`init: mode ${this.mode.toString()} to ${this.host}:${this.port}`);
  }
  /**
   * Log a debug message
